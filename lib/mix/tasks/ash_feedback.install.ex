if Code.ensure_loaded?(Igniter) do
  defmodule Mix.Tasks.AshFeedback.Install do
    ...
    # AshPaperTrail-generated Version companion, Comment, Blob,
    # Attachment) to the domain's `resources` block.
    ...
    defp add_resources_to_domain(igniter) do
      ...
      [
        ...
        feedback_comment_module(igniter),
    ...
    # Same pattern as build_use_feedback_opts/6 — only emits the
    # author_resource and pubsub opts when their host modules exist.
    defp build_use_feedback_comment_opts(igniter, app_name, domain, repo, entry) do
      ...
      lines =
        [
          ...
          if(user_exists?, do: "  author_resource: #{inspect(user_module)}"),
          if(pubsub_exists?, do: "  pubsub: #{inspect(pubsub_module)}")
        ]
        |> Enum.reject(&is_nil/1)
        |> Enum.join(",\n")
