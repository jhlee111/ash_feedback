defmodule AshFeedback.Storage do
  ...
  Policies, `AshPaperTrail`, and tenant scoping on the host's domain
  fire on every call.
  """
  ...
  def submit(session_id, params, identity) do
    ...
    attrs = maybe_forward_audio_clip_blob_id(attrs, params)
  ...
  # action argument when present — every Feedback resource generated by
  # `AshFeedback.Resources.Feedback` accepts it (audio is core).
  defp maybe_forward_audio_clip_blob_id(attrs, params) do
    blob_id =
      params
      |> Map.get("extras", %{})
      |> Map.get("audio_clip_blob_id")

    if is_binary(blob_id) do
